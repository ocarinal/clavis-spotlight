...
`smart`、`most-used`、`recently-used`、`name`；新配置和缺失值默认 Most used，
非法显式值回退 Name。`spotlightPinnedAppIds` 保存最多 128 个去重的
desktop ID，右键应用可在列表和网格中切换置顶。
列表和网格共用排序；搜索时关键词相关度始终优先，使用排序只比较相同相关度，
相同相关度中置顶应用按保存顺序优先，再比较使用记录，最后按名称、desktop ID
确定稳定顺序。
...
右键应用图标可置顶、隐藏或恢复显示；`spotlightHiddenAppIds` 保存隐藏的应用，
被隐藏的应用不再出现在任何提供者中，只有右键菜单打开“显示隐藏的图标”时才回到列表
并标记为隐藏。长按图标进入手工排序：拖动结束把当前完整顺序写入
`spotlightManualAppOrder`（最多 512 项），排序链为相关度、置顶、手工顺序、已选排序、
名称；右键菜单的“恢复自动排序”清空该列表。手工排序只在完整列表（网格、无查询）中
允许，避免把过滤后的局部顺序写回。

## 独立 Spotlight 的桌面壁纸

独立 Spotlight 使用隔离的 Clavis 配置，`WallpaperService` 的写入只影响该配置，
不会改变正在渲染的桌面。桌面壁纸与配色由本会话的 Noctalia 负责，因此壁纸页读取
当前壁纸和点击应用都通过 `NoctaliaWallpaperService` 调用
`noctalia msg wallpaper-get` / `wallpaper-set`；命令可用 `CLAVIS_NOCTALIA` 覆盖。
应用成功后面板保持打开，当前壁纸勾选标记移到新壁纸；失败时由面板标题行说明原因，
不静默改写状态。Escape 逐层返回：先清空输入，再回到打开表面时的模式并展开模式栏
（四个模式按钮属于这一层），最后才退出表面。
