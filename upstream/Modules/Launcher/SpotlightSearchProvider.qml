...
Item {
    ...
    function rebuild() {
        ...
        const visible = LocalSearch.visibleApps(ApplicationService.launcherApplications,
                                                UiPreferences.spotlightHiddenAppIds, false);
        const apps = LocalSearch.appResults(visible, query, UiPreferences.spotlightAppOrder,
                                            SpotlightAppUsage.records, Date.now(),
                                            UiPreferences.spotlightPinnedAppIds,
                                            UiPreferences.spotlightManualAppOrder).map(entry => Object.assign(
                                                                                                    {}, entry,
                                                                                                    {
    ...
    function activate(id) {
        ...
        case "apps":
            ...
            break;
        case "wallpapers":
            if (WallpaperService.wallpapers.indexOf(request.sourceId) < 0)
                break;
            // Applying goes through the desktop owner; the launcher closes
            // once that service confirms the change.
            return NoctaliaWallpaperService.apply(request.sourceId);
        case "settings":
