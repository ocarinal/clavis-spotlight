...
Item {
    ...
    readonly property int wallpaperHeaderHeight: mode === "wallpapers" && wallpaperError !== "" ? 32 : 0
    readonly property int headerHeight: fileHeaderHeight + wallpaperHeaderHeight
    ...
    property string appContextId: ""
    readonly property bool appContextPinned: appContextId !== "" && UiPreferences.isSpotlightAppPinned(
                                                 appContextId)
    readonly property bool appContextHidden: appContextId !== "" && UiPreferences.isSpotlightAppHidden(
                                                 appContextId)
    readonly property bool hasHiddenApps: UiPreferences.spotlightHiddenAppIds.length > 0
    readonly property bool hasManualAppOrder: UiPreferences.spotlightManualAppOrder.length > 0
    property bool showHiddenApps: false
    readonly property bool modalActive: clearDialog.visible || fileMenuActive || appContextMenu.visible
    readonly property int targetHeight: {
        ...
        if (loading || !providerAvailable || results.length === 0)
            return Math.min(availableHeight, style.emptyHeight + clipboardHeaderHeight + headerHeight);
        ...
        if (mode === "wallpapers")
            return Math.min(style.wallpaperGridHeight + wallpaperHeaderHeight, Math.max(0, availableHeight));
        return Math.min(availableHeight, style.resultMaxHeight, results.length * style.resultRowHeight
                        + style.resultPadding * 2 + clipboardHeaderHeight + headerHeight);
    }
    ...
    signal hiddenAppsToggleRequested
    signal appReorderRequested(string fromId, string toId)

    height: targetHeight
    opacity: expanded ? revealProgress : 0
    scale: 0.965 + 0.035 * revealProgress
    transformOrigin: Item.Top
    transform: Translate {
        y: 14 * (1 - root.revealProgress)
    }
    ...
    Behavior on opacity {
        enabled: root.animationsEnabled && root.revealProgress >= 0.999
        NumberAnimation {
            duration: root.style.panelDuration
            easing.type: Easing.BezierSpline
    ...
    function openAppContext(index, sourceItem) {
        if (root.mode !== "apps" || index < 0 || index >= root.results.length)
            return false;
        const result = root.results[index];
        if (!result || !result.appObject || result.appObject.dragOnly)
            return false;
        const id = String(result.appObject.id || result.id || "").trim();
        if (id === "")
            return false;
        root.appContextId = id;
        const point = sourceItem ? sourceItem.mapToItem(root, sourceItem.width / 2, sourceItem.height / 2) :
                                   Qt.point(root.width / 2, 0);
        appContextMenu.x = Math.max(8, Math.min(root.width - appContextMenu.width - 8, point.x));
        appContextMenu.y = Math.max(8, Math.min(root.height - appContextMenu.implicitHeight - 8, point.y));
        appContextMenu.open();
        return true;
    }
    ...
    Rectangle {
        ...
        color: "transparent"
        border.width: 1
        border.color: root.style.glassBorderColor

        gradient: Gradient {
            orientation: Gradient.Vertical
            GradientStop {
                position: 0
                color: root.style.glassTopHighlightColor
            }
            GradientStop {
                position: 0.32
                color: "transparent"
            ...
            GradientStop {
                position: 1
                color: root.style.glassBottomShadeColor
            }
        }
    }
    ...
    SpotlightAppGrid {
        ...
        onContextRequested: (index, sourceItem) => root.openAppContext(index, sourceItem)
        onReorderRequested: (from, to) => root.appReorderRequested(from, to)
    }
    ...
    StackLayout {
        visible: root.mode !== "search"
        anchors.fill: parent
        anchors.topMargin: root.headerHeight + root.style.resultPadding
            ...
            ListView {
                ...
                delegate: Item {
                    ...
                    RowLayout {
                        ...
                        MaterialSymbol {
                            visible: root.mode === "apps" && UiPreferences.isSpotlightAppHidden(
                                         appDelegate.modelData.id)
                            text: "visibility_off"
                            iconSize: 19
                            color: Appearance.colors.colOnSurfaceVariant
                        }
                    ...
                    MouseArea {
                        ...
                        acceptedButtons: root.fileMode || root.mode === "apps" ? Qt.LeftButton
                                                                                 | Qt.RightButton :
                                                                                 Qt.LeftButton
                        ...
                        onClicked: mouse => {
                            ...
                            if (mouse.button === Qt.RightButton) {
                                if (root.fileMode)
                                    ...
                                    root.openAppContext(appDelegate.index, appDelegate);
                            } else {
        ...
        GridView {
            ...
            delegate: Item {
                ...
                readonly property bool isCurrentWallpaper: root.currentWallpaperPath !== ""
    ...
    StyledMenu {
        ...
        StyledMenuItem {
            iconName: "push_pin"
            text: root.appContextPinned ? qsTr("Unpin app") : qsTr("Pin app")
            checkable: true
            checked: root.appContextPinned
            onTriggered: UiPreferences.toggleSpotlightAppPinned(root.appContextId)
        }
        StyledMenuItem {
            iconName: root.appContextHidden ? "visibility" : "visibility_off"
            text: root.appContextHidden ? qsTr("Unhide app") : qsTr("Hide app")
            onTriggered: UiPreferences.setSpotlightAppHidden(root.appContextId, !root.appContextHidden)
        }
        StyledMenuItem {
            iconName: "visibility"
            text: qsTr("Show hidden apps")
            checkable: true
            checked: root.showHiddenApps
            visible: root.hasHiddenApps
            onTriggered: root.hiddenAppsToggleRequested()
        }
        StyledMenuItem {
            iconName: "sort"
            text: qsTr("Reset app order")
            visible: root.hasManualAppOrder
            onTriggered: UiPreferences.clearSpotlightManualAppOrder()
        }
    }
    ...
    Text {
        ...
        text: root.wallpaperError !== "" ? root.wallpaperError : root.fileError ? root.fileError.message :
                                                                                  root.fileState
                                                                                  === "limited" ? qsTr(
        ...
        font.family: Fonts.ui
        font.pixelSize: 12
        color: root.fileError || root.wallpaperError !== "" ? Appearance.colors.colError :
                                                              Appearance.colors.colOnSurfaceVariant
    }
