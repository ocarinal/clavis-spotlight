...
PanelWindow {
    ...
    // Mode the surface opened in. Escape walks back to it before closing.
    property string openedMode: "apps"
    // The opened level with nothing pending in it.
    readonly property bool atOpenedLevel: root.mode === root.openedMode && !session.tool && root.query === ""
    ...
    property real _panelRevealAnimationTarget: 0
    ...
    Connections {
        ...
        function onApplyFailed(path, message) {
            searchBar.flashError();
        }
    }
    ...
    function modeForIndex(index) {
        return ["web", "wallpapers", "clipboard", "files"][Math.max(0, Math.min(style.modeButtonCount - 1,
                                                                                index))];
    }

    function activateModeIndex(index) {
        const destination = root.modeForIndex(index);
        if (destination === "web")
            return root.enterWeb();
        const activated = root.setLocalMode(destination);
        root.setRailExpanded(false);
        return activated;
    }
    ...
    function animatePanelReveal(target) {
        root._panelRevealAnimationTarget = target;
        panelRevealAnimation.stop();
        panelRevealAnimation.from = root.panelRevealProgress;
        panelRevealAnimation.to = target;
        panelRevealAnimation.duration = Math.max(1, (target > root.panelRevealProgress
                                                     ? style.panelRevealDuration : style.panelHideDuration)
                                                 * Math.abs(target - root.panelRevealProgress));
        panelRevealAnimation.restart();
    }
    ...
    function exitWeb() {
        return session.tool === "web" && root.popLevel();
    }
    ...
    function activateResult(index, keepClipboardOpen) {
        ...
        if (!root.selectResult(index))
            return false;
        // Pointer activation must use the delegate's exact index. Routing it
        // through the keyboard selection path lets rail focus or a pending
        // selection update consume the click before the application launches.
        if (root.mode === "apps") {
            if (!appProvider.execute(index))
                return false;
            root.requestClose();
            return true;
        }
    ...
    function activateSelected(keepClipboardOpen) {
        ...
        if (root.modeRailExpanded && root.modeFocusIndex >= 0) {
            return root.activateModeIndex(root.modeFocusIndex);
        }
        if (session.slashDraft)
    ...
    // Leaving a mode or a tool returns to the level the surface opened on.
    // That level owns the four mode buttons, so bring the rail back with it.
    function restoreOpenedRail() {
        if (root.mode === root.openedMode)
            root.setRailExpanded(true);
    }

    // Typing on the opened level hands the query to the unified search, which
    // answers with apps, settings, actions, wallpapers and the file/web rows.
    // Clearing the input returns to the level the surface opened on.
    function syncQueryMode() {
        if (root.windowPhase !== "open" || session.slashDraft || root.toolMode)
            return;
        if (root.query.trim() !== "") {
            if (root.mode === "apps" && root.openedMode === "apps")
                root.setLocalMode("search");
            return;
        }
        if (root.mode === "search" && root.openedMode === "apps")
            root.setLocalMode("apps");
        root.restoreOpenedRail();
    }

    // One step back, by whichever control asks for it: the pill's close button,
    // Backspace, Escape or the currency editor.
    function popLevel() {
    ...
    function handleEscape() {
        if (root.spotlightModalActive)
            return;
        if (root.atOpenedLevel) {
            // Escape walks back one level at a time; the opened level closes
            // the surface instead of leaving it without its mode buttons.
            root.requestClose();
            ...
            root.query = "";
        } else if (root.mode !== root.openedMode) {
            root.setLocalMode(root.openedMode);
            root.restoreOpenedRail();
        } else {
            root.requestClose();
        }
    ...
    NumberAnimation {
        id: panelRevealAnimation

        target: root
        property: "panelRevealProgress"
        easing.type: Easing.BezierSpline
        easing.bezierCurve: root._panelRevealAnimationTarget > root.panelRevealProgress
    ...
    FocusScope {
        ...
        SpotlightSearchBar {
            ...
            onCurrencyExitRequested: root.popLevel()
            mode: root.mode
        ...
        SpotlightResultsPanel {
            ...
            revealProgress: root.panelRevealProgress
            ...
            currentWallpaperPath: NoctaliaWallpaperService.currentPath
            wallpaperError: NoctaliaWallpaperService.lastError
            showHiddenApps: appProvider.showHidden
            availableHeight: Math.max(0, root.height - spotlightRoot.baseY - searchBar.height
                                      - style.resultGap - style.windowBottomMargin)
            ...
            onHiddenAppsToggleRequested: appProvider.showHidden = !appProvider.showHidden
            onAppReorderRequested: (fromId, toId) => appProvider.reorder(fromId, toId)
        }
    }
}
