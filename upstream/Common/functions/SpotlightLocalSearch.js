...
function hiddenIds(value) {
    const result = Object.create(null);
    AppOrder.normalizedAppIds(value).forEach(id => result[id] = true);
    return result;
}

// Apps the user hid from the launcher stay out of every provider until the
// caller explicitly asks to manage them.
function visibleApps(source, hidden, showHidden) {
    if (showHidden) return Array.from(source || []);
    const excluded = hiddenIds(hidden);
    return Array.from(source || []).filter(app => !app || !excluded[String(app.id)]);
}

function appResults(source, query, order, history, now, pinnedIds, manualOrder) {
    ...
    return AppOrder.sortedResults(next, order, history, now, pinnedIds, manualOrder);
}
