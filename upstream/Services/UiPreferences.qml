...
Singleton {
    ...
    property string spotlightAppOrder: "most-used"
    property var spotlightPinnedAppIds: []
    property var spotlightHiddenAppIds: []
    property var spotlightManualAppOrder: []
    ...
    function isSpotlightAppPinned(value) {
        const normalized = AppOrder.normalizedAppIds([value]);
        return normalized.length > 0 && root.spotlightPinnedAppIds.indexOf(normalized[0]) >= 0;
    }

    function setSpotlightAppPinned(value, pinned) {
        const normalized = AppOrder.normalizedAppIds([value]);
        if (normalized.length === 0)
            return false;
        const id = normalized[0];
        const current = root.spotlightPinnedAppIds.indexOf(id);
        if (!!pinned === (current >= 0))
            return true;
        if (pinned && root.spotlightPinnedAppIds.length >= 128)
            return false;
        root.spotlightPinnedAppIds = pinned ? root.spotlightPinnedAppIds.concat([id]) :
                                              root.spotlightPinnedAppIds.filter(entry => entry !== id);
        root.save();
        return true;
    }

    function toggleSpotlightAppPinned(value) {
        return root.setSpotlightAppPinned(value, !root.isSpotlightAppPinned(value));
    }

    function isSpotlightAppHidden(value) {
        const normalized = AppOrder.normalizedAppIds([value]);
        return normalized.length > 0 && root.spotlightHiddenAppIds.indexOf(normalized[0]) >= 0;
    }

    function setSpotlightAppHidden(value, hidden) {
        const normalized = AppOrder.normalizedAppIds([value]);
        if (normalized.length === 0)
            return false;
        const id = normalized[0];
        const current = root.spotlightHiddenAppIds.indexOf(id);
        if (!!hidden === (current >= 0))
            return true;
        if (hidden && root.spotlightHiddenAppIds.length >= 128)
            return false;
        root.spotlightHiddenAppIds = hidden ? root.spotlightHiddenAppIds.concat([id]) :
                                              root.spotlightHiddenAppIds.filter(entry => entry !== id);
        // A hidden app is no longer reachable, so it must not stay pinned.
        if (hidden && root.spotlightPinnedAppIds.indexOf(id) >= 0)
            root.setSpotlightAppPinned(id, false);
        root.save();
        return true;
    }

    function toggleSpotlightAppHidden(value) {
        return root.setSpotlightAppHidden(value, !root.isSpotlightAppHidden(value));
    }

    function setSpotlightManualAppOrder(value) {
        const next = AppOrder.normalizedAppIds(value, 512);
        if (next.length === 0)
            return false;
        if (next.join("\n") === root.spotlightManualAppOrder.join("\n"))
            return true;
        root.spotlightManualAppOrder = next;
        root.save();
        return true;
    }

    function clearSpotlightManualAppOrder() {
        if (root.spotlightManualAppOrder.length === 0)
            return true;
        root.spotlightManualAppOrder = [];
        root.save();
        return true;
    }
    ...
    function save() {
        ...
        prefsFile.setText(JSON.stringify({
                                             ...
                                             "spotlightPinnedAppIds": root.spotlightPinnedAppIds,
                                             "spotlightHiddenAppIds": root.spotlightHiddenAppIds,
                                             "spotlightManualAppOrder": root.spotlightManualAppOrder,
    ...
    FileView {
        ...
        onLoaded: {
            try {
                ...
                root.spotlightAppOrder = parsed.spotlightAppOrder === undefined ? "most-used" :
                                                                                  AppOrder.normalizedOrder(
                                                                                      parsed.spotlightAppOrder);
                root.spotlightPinnedAppIds = AppOrder.normalizedAppIds(parsed.spotlightPinnedAppIds);
                root.spotlightHiddenAppIds = AppOrder.normalizedAppIds(parsed.spotlightHiddenAppIds);
                root.spotlightManualAppOrder = AppOrder.normalizedAppIds(parsed.spotlightManualAppOrder, 512);
